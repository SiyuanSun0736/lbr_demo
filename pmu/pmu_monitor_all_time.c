...
// Sampling interval in milliseconds (change here to adjust frequency)
...
int main(int argc, char **argv) {
    ...
    char link_name[] = "log/pmu_monitor_all_time.log";
    snprintf(timestamped_filename, sizeof(timestamped_filename), 
             "log/pmu_monitor_all_time_%04d%02d%02d_%02d%02d%02d.log",
    ...
    unlink(link_name);
    char target[256];
    snprintf(target, sizeof(target), "pmu_monitor_all_time_%04d%02d%02d_%02d%02d%02d.log",
             t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
             t->tm_hour, t->tm_min, t->tm_sec);
    ...
    /* Removed L1-icache-loads counter per request; keep miss counter only */
    ...
    // Set up timerfd for low-jitter periodic sampling (CLOCK_MONOTONIC)
    ...
    struct itimerspec its = {
        .it_interval = { .tv_sec  = SAMPLE_INTERVAL_MS / 1000,
                         .tv_nsec = (long)(SAMPLE_INTERVAL_MS % 1000) * 1000000L },
        .it_value    = { .tv_sec  = SAMPLE_INTERVAL_MS / 1000,
                         .tv_nsec = (long)(SAMPLE_INTERVAL_MS % 1000) * 1000000L },
    };
    ...
    while (1) {
        // Block until next timer tick; CLOCK_MONOTONIC ensures no drift accumulation
        ...
        if (expirations > 1) {
            fprintf(stderr, "Warning: timer overrun — %lu interval(s) missed\n",
                    expirations - 1);
        }
