...
"$SP_BIN" -p "$SAMPLE_PERIOD_FAST" \
    >"log/sp_stdout_syswide_$$.txt" 2>"log/sp_stderr_syswide_$$.txt" &
SP_PID=$!
info "采集器 PID: $SP_PID"
...
"$SP_BIN" "$WORKLOAD_PID" -p "$SAMPLE_PERIOD_SLOW" \
    >"log/sp_stdout_pid_$$.txt" 2>"log/sp_stderr_pid_$$.txt" &
SP_PID=$!
info "采集器 PID: $SP_PID"
...
rm -f "log/sp_stdout_syswide_$$.txt" "log/sp_stderr_syswide_$$.txt" \
      "log/sp_stdout_pid_$$.txt"     "log/sp_stderr_pid_$$.txt"
